...
ax = plt.subplot2grid((rows, 2), (6, 0), colspan=2, rowspan=10)
...
plt.gca().xaxis.grid(which='minor', linestyle='dashed')
plt.gca().set_xticks(np.linspace(latticedata.LatticeLength / 32, latticedata.LatticeLength * (1 - 1 / 32), 16, endpoint=True), minor=True)
plt.gca().set_xticks(np.linspace(start_sec[0], end_sec[1], 13, endpoint=True))
plt.gca().yaxis.grid(linestyle='dashed')
plt.gca().set_xlabel('orbit position $s$ / m')
plt.gca().set_yticks(np.linspace(0, 24, 7, endpoint=True))

# limits
plt.xlim(start_pos[0], end_pos[1])
plt.ylim(ylim_min, ylim_max)
paintlattice(start_pos[0], end_pos[1], latticedata, ylim_min, ylim_max, halfsize=True, noborder=True)

plt.annotate("Doublet", xy=(start_pos[0] + 8, 0.89 * ylim_max), fontsize=15, va='center', ha='center', clip_on=True, zorder=102, weight='bold')

plt.annotate("Triplet", xy=(start_pos[1] + 8, 0.89 * ylim_max), fontsize=15, va='center', ha='center', clip_on=True, zorder=102, weight='bold')
# plt.annotate("DBA", xy=(start_sec[0] + 15, 0.89 * ylim_max), fontsize=15, va='center', ha='center', clip_on=True, zorder=102, weight='bold')

labels = plt.gca().get_xticks().tolist()
labels = [label - start_sec[0] for label in labels]
...
# save fig
plt.tight_layout()
plt.savefig("../../images/04-design-lattice-def.pdf", transparent=True)
