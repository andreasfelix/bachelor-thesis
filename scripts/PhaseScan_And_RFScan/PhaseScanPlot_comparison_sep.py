...
directory = 'data_04_09_2017/'
for j, filepath in enumerate(os.listdir(directory)):
    filepath = directory + filepath

    data = np.loadtxt(filepath, usecols=(3, 5), delimiter="\t", skiprows=1)

    index_array = np.argsort(data[:, 0])
    phase = data[:, 0][index_array]
    efficiency = data[:, 1][index_array]
    ...
    ax.plot(phasenew, efficiencynew, '-', lw=2, color=cmap(j / 9), label=os.path.basename(filepath).split(".")[0][17:-13])
...
plt.savefig('../../images/05-comparison-phase-acceptance_sep.pdf')
